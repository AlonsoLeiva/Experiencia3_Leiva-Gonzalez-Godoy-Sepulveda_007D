function mostrar(){
    document.getElementById('comprar').style.visibility='hidden';
    document.getElementById('imagen').style.display='none';
    document.getElementById('datos').style.display= 'block';
    document.getElementById('boton').style.visibility='hidden';
    document.getElementById('titulo').style.display='none';
  
  }
  
  function ocultar(){
    document.getElementById('comprar').style.visibility='visible';
    document.getElementById('datos').style.display ='none';
    document.getElementById('imagen').style.display = 'block';
    document.getElementById('boton').style.visibility='visible';
    document.getElementById('titulo').style.display='block';
  }
  function mostrar2(){
      document.getElementById('comprar2').style.visibility='hidden';
      document.getElementById('imagen2').style.display='none'
      document.getElementById('datos2').style.display= 'block';
      document.getElementById('boton3').style.visibility='hidden';
      document.getElementById('titulo2').style.display='none';
    
    }
    
    function ocultar2(){
    
      document.getElementById('datos2').style.display ='none';
      document.getElementById('imagen2').style.display = 'block';
      document.getElementById('boton3').style.visibility='visible';
      document.getElementById('titulo2').style.display='block';
      document.getElementById('comprar2').style.visibility='visible';
    }
  
    function mostrar3(){
      document.getElementById('comprar3').style.visibility='hidden';
      document.getElementById('imagen3').style.display='none'
      document.getElementById('datos3').style.display= 'block';
      document.getElementById('boton5').style.visibility='hidden';
      document.getElementById('titulo3').style.display='none';
    
    }
    
    function ocultar3(){
      document.getElementById('datos3').style.display ='none';
      document.getElementById('imagen3').style.display = 'block';
      document.getElementById('boton5').style.visibility='visible';
      document.getElementById('titulo3').style.display='block';
      document.getElementById('comprar3').style.visibility='visible';
    }
  
    function mostrar4(){
      document.getElementById('comprar4').style.visibility='hidden';
      document.getElementById('imagen4').style.display='none'
      document.getElementById('datos4').style.display= 'block';
      document.getElementById('boton7').style.visibility='hidden';
      document.getElementById('titulo4').style.display='none';
    
    }
    
    function ocultar4(){
      document.getElementById('datos4').style.display ='none';
      document.getElementById('imagen4').style.display = 'block';
      document.getElementById('boton7').style.visibility='visible';
      document.getElementById('titulo4').style.display='block';
      document.getElementById('comprar4').style.visibility='visible';
    }
  
  
  function mostrar5(){
      document.getElementById('comprar5').style.visibility='hidden';
      document.getElementById('imagen5').style.display='none'
      document.getElementById('datos5').style.display= 'block';
      document.getElementById('boton9').style.visibility='hidden';
      document.getElementById('titulo5').style.display='none';
    
    }
    
  function ocultar5(){
      document.getElementById('datos5').style.display ='none';
      document.getElementById('imagen5').style.display = 'block';
      document.getElementById('boton9').style.visibility='visible';
      document.getElementById('titulo5').style.display='block';
      document.getElementById('comprar5').style.visibility='visible';
    }
  
    function mostrar6(){
      document.getElementById('comprar6').style.visibility='hidden';
      document.getElementById('imagen6').style.display='none'
      document.getElementById('datos6').style.display= 'block';
      document.getElementById('boton11').style.visibility='hidden';
      document.getElementById('titulo6').style.display='none';
    
    }
    
  function ocultar6(){
      document.getElementById('datos6').style.display ='none';
      document.getElementById('imagen6').style.display = 'block';
      document.getElementById('boton11').style.visibility='visible';
      document.getElementById('titulo6').style.display='block';
      document.getElementById('comprar6').style.visibility='visible';
    }
  
    function mostrar7(){
      document.getElementById('comprar7').style.visibility='hidden';
      document.getElementById('imagen7').style.display='none'
      document.getElementById('datos7').style.display= 'block';
      document.getElementById('boton13').style.visibility='hidden';
      document.getElementById('titulo7').style.display='none';
    
    }
    
  function ocultar7(){
      document.getElementById('datos7').style.display ='none';
      document.getElementById('imagen7').style.display = 'block';
      document.getElementById('boton13').style.visibility='visible';
      document.getElementById('titulo7').style.display='block';
      document.getElementById('comprar7').style.visibility='visible';
    }
    
    function mostrar8(){
      document.getElementById('comprar8').style.visibility='hidden';
      document.getElementById('imagen8').style.display='none'
      document.getElementById('datos8').style.display= 'block';
      document.getElementById('boton15').style.visibility='hidden';
      document.getElementById('titulo8').style.display='none';
    
    }
    
  function ocultar8(){
      document.getElementById('datos8').style.display ='none';
      document.getElementById('imagen8').style.display = 'block';
      document.getElementById('boton15').style.visibility='visible';
      document.getElementById('titulo8').style.display='block';
      document.getElementById('comprar8').style.visibility='visible';
    }
    function mostrar9(){
      document.getElementById('comprar9').style.visibility='hidden';
      document.getElementById('imagen9').style.display='none'
      document.getElementById('datos9').style.display= 'block';
      document.getElementById('boton17').style.visibility='hidden';
      document.getElementById('titulo9').style.display='none';
    
    }
    
  function ocultar9(){
      document.getElementById('comprar9').style.visibility='visible';
      document.getElementById('datos9').style.display ='none';
      document.getElementById('imagen9').style.display = 'block';
      document.getElementById('boton17').style.visibility='visible';
      document.getElementById('titulo9').style.display='block';
    }